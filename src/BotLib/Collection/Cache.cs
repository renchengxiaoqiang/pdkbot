...
namespace PdkBot.BotLib.Collection
{
    public class Cache<KT, VT>
    {
        ...
        public VT this[KT key]
        {
            ...
            {
                VT result;
                TryGetValue(key, out result, default);
                return result;
            }
            set
            {
        ...
        public bool TryGetValue(KT key, out VT value, VT defv = default)
        {
            accessCount += 1L;
            value = defv;
            bool rt = false;
        ...
        public VT GetValue(KT key, Func<VT> producer, bool useCache = true, Predicate<VT> needCache = null)
        {
            VT vt = default;
            accessCount += 1L;
            if (useCache && TryGetValue(key, out vt, default))
            {
                hitCount += 1L;
            }
            else
        ...
        public double HitRate()
        {
            ...
            {
                rt = hitCount / (double)accessCount;
            }
            return rt;
        }
                    ...
                    if (_dict.Count > _maxCacheSize)
                    {
                        try
                        {
                            ...
                            dict.OrderByDescending(k => k.Value.LastAccessTime)
                                .Skip(_minCacheSize).Select(k => k.Key).ToList()
                                .ForEach(k =>
                                {
                                    Data data;
        ...
        private class Data
        {
            ...
            {
                return timeoutMs > 0 && (DateTime.Now - CacheTime).TotalMilliseconds > timeoutMs;
            }
